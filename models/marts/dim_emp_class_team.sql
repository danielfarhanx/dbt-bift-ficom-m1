{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1_airbyte.dim_emp_class_team__dbt_backup drop constraint if exists dim_emp_class_team_pkey {% else %} select 1 {% endif %}",
