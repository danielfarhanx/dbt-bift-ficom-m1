{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1_airbyte.dim_m_group_salesforce__dbt_backup drop constraint if exists dim_m_group_salesforce_pk {% else %} select 1 {% endif %}",
