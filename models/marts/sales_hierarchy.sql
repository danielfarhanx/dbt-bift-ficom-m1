{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1_airbyte.sales_hierarchy__dbt_backup drop constraint if exists sales_hierarchy_pk {% else %} select 1 {% endif %}",
