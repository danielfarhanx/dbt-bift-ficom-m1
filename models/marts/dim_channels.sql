{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1_airbyte.dim_channels__dbt_backup drop constraint if exists dim_channels_pkey {% else %} select 1 {% endif %}",
