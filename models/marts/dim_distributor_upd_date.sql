{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1.dim_distributor_upd_date__dbt_backup drop constraint if exists dim_distributor_upd_date_pkey {% else %} select 1 {% endif %}",
