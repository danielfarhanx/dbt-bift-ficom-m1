{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1_airbyte.dim_emp_mkt_sales__dbt_backup drop constraint if exists dim_emp_mkt_sales_pkey {% else %} select 1 {% endif %}",
