{{ config(
    ...
    post_hook=[
        "{% if not is_incremental() %} alter table if exists starscm_m1_airbyte.dim_employe_help__dbt_backup drop constraint if exists dim_employe_help_pkey {% else %} select 1 {% endif %}",
